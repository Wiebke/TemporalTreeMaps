...
std::shared_ptr<TemporalTree> readJSONTree(const std::string& filePath, const int Type) {
    ...
    if (!filesystem::fileExists(Filename)) {
        ...
        if (filesystem::fileExists(NewPath)) {
            ...
            throw DataReaderException("Could not find input file: " + Filename,
                                      IvwContextCustom("JSONReader"));
        }
    }

    // Open the file
    std::ifstream InFile;
    // InFile.exceptions(std::ifstream::failbit | std::ifstream::badbit); //From the json
    // documentation: Please note that setting the exception bit for failbit is inappropriate for
    // this use case. It will result in program termination due to the noexcept specifier in use.
    try {
...
TemporalTreeJSONReader::TemporalTreeJSONReader() : DataReaderType<TemporalTree>() {
...
TemporalTreeJSONReaderCBOR::TemporalTreeJSONReaderCBOR() : DataReaderType<TemporalTree>() {
...
TemporalTreeJSONReaderCBOR& TemporalTreeJSONReaderCBOR::operator=(
    const TemporalTreeJSONReaderCBOR& that) {
    if (this != &that) {
        DataReaderType<TemporalTree>::operator=(that);
    }
...
TemporalTreeJSONReaderMsgPack::TemporalTreeJSONReaderMsgPack() : DataReaderType<TemporalTree>() {
...
TemporalTreeJSONReaderMsgPack::TemporalTreeJSONReaderMsgPack(
    const TemporalTreeJSONReaderMsgPack& rhs)
    : DataReaderType<TemporalTree>(rhs) {}

TemporalTreeJSONReaderMsgPack& TemporalTreeJSONReaderMsgPack::operator=(
    const TemporalTreeJSONReaderMsgPack& that) {
    if (this != &that) {
        DataReaderType<TemporalTree>::operator=(that);
    }
...
TemporalTreeJSONReaderNTG::TemporalTreeJSONReaderNTG() : DataReaderType<TemporalTree>() {
...
TemporalTreeJSONReaderNTG& TemporalTreeJSONReaderNTG::operator=(
    const TemporalTreeJSONReaderNTG& that) {
    if (this != &that) {
        DataReaderType<TemporalTree>::operator=(that);
    }
...
bool ReadEdgesNTG(const json& j, const std::string& Name,
    ...
    if (j.cend() != j.find(Name)) {
        ...
        const json& jEdges = j[Name];
        for (json::const_iterator ignore = jEdges.cbegin(); ignore != jEdges.cend() && !bError;
             ignore++) {
            for (json::const_iterator it = ignore.value().cbegin();
                 it != ignore.value().cend() && !bError; it++) {
                ...
                if (NodeNameToIndex.count(FromNode)) {
                    ...
                    if (!bError) {
                        // EdgeBuffer[idxFromNode].insert(EdgeBuffer[idxFromNode].end(),
                        // ToNodeIndices.begin(), ToNodeIndices.end());
...
std::shared_ptr<TemporalTree> TemporalTreeJSONReaderNTG::readData(const std::string& filePath) {
    ...
    if (!filesystem::fileExists(Filename)) {
        ...
        if (filesystem::fileExists(NewPath)) {
            ...
            throw DataReaderException("Could not find input file: " + Filename,
                                      IvwContextCustom("TemporalTreeJSONReaderNTG"));
        }
    }
    ...
    if (j.cend() != j.find("N")) {
        ...
        for (json::const_iterator it = jNodes.cbegin(); it != jNodes.cend(); it++) {
            ...
            // Get the value, which is a json object containing time step (t), data (w), and other
            // unused info
            ...
            ThisNode.values.emplace(Time + 1,
                                    DataValue);  // To make it consistent with our data structure
    ...
    bOKWhileReadingEdges =
        bOKWhileReadingEdges && ReadEdgesNTG(j, "EN", NodeNameToIndex, pTree->edgesHierarchy);
    bOKWhileReadingEdges =
        bOKWhileReadingEdges && ReadEdgesNTG(j, "ET", NodeNameToIndex, pTree->edgesTime);

    if (!bOKWhileReadingEdges) {
        throw DataReaderException("Could not read edges!",
                                  IvwContextCustom("TemporalTreeJSONReaderNTG"));
    }
