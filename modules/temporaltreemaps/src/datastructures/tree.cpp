...
/*
    TemporalTree TemporalTree::getHierarchyAt(uint64_t time, bool accumulate = false, uint64_t
   deltaTime = 1) const
    {
        ...
        auto copyRelevantNodeAttributes = [&](const TemporalTree::TNode& node) ->
   TemporalTree::TNode
        {
            ...
            for (auto edge : it->second)
            {
                ...
                if (time >= edge.startTime && time < edge.endTime)
                {
                    ...
                    if (indexPairFrom == indexMap.end())
                    {
                        ...
                        indexPairFrom = indexMap.insert(std::make_pair(edgeFrom,
   snapshotNodes.size() - 1)).first;
                    }
                    ...
                    if (indexPairTo == indexMap.end())
                    {
                        ...
                        snapshotNodes[snapshotNodes.size() - 1].index = snapshotNodes.size() - 1;
                        indexPairTo = indexMap.insert(std::make_pair(edge.to, snapshotNodes.size() -
   1)).first;
                    }
                    // Add edge with the snapshot indices
                    snapshot.addHierarchyEdge(indexPairFrom->second, indexPairTo->second, time, time
   + deltaTime); } // Active edge } // Outgoing edges for one node
        }
    ...
    while (currentLevel < level) {
        ...
        for (const auto parentIndex : currentLevelIndices) {
            std::map<size_t, std::vector<size_t>>::const_iterator it =
                edgesHierarchy.find(parentIndex);
...
void TemporalTree::leafDepthStatistic(const size_t nodeIndex, std::vector<bool>& visited,
...
void TemporalTree::splitTemporaryLeaves() {
    ...
    for (size_t nodeIndex(1); nodeIndex < numNodes; nodeIndex++) {
        ...
        getLeaves(nodeIndex, node.startTime(), node.endTime(), node.startTime(), node.endTime(),
                  leafIndices);
        ...
        for (auto leafIndex : leafIndices) {
            auto itStartTimeChild =
                times.find(std::max(node.startTime(), nodes[leafIndex].startTime()));
            const auto itOneAfterEndTimeChild =
                std::next(times.find(std::min(node.endTime(), nodes[leafIndex].endTime())));
        ...
        if (std::distance(itStartTimeNode, itEndTimeNode) != int(hasLeaves.size())) {
            ...
            for (auto it = itStartTimeNode; it != itEndTimeNode; it++) {
                ...
                if (stateBefore == 0 || (itIsLeaf != hasLeaves.end() && stateBefore == 1) ||
                    ...
                    // We need to start a new node (a leaf) and also give it this value and the
                    // previous one Previous one will always exists, there cannot be a one value
                    // leaf that later becomes a non-leaf or vice versa (because when would it?)
            ...
            if (newNodes.size() >= 2) {
                ...
                // Everything that points to the node, will now point to the first of the new nodes
                // (Fine) Everything that the node pointed to, the last new node needs to point to
                ...
                for (auto parent : parents) {
                    // Parents were found through hierarchy edges, so we can safely assume them to
                    // be there
                    ...
                    for (size_t index = 0; index < newNodes.size(); index++) {
                        const uint64_t overlapStart =
                            std::max(parentStartTime, newNodes[index].startTime());
                        const uint64_t overlapEnd =
                            std::min(parentEndTime, newNodes[index].endTime());
                ...
                for (auto child : children) {
                    ...
                    for (size_t index = 0; index < newNodes.size(); index++) {
                        const uint64_t overlapStart =
                            std::max(childStartTime, newNodes[index].startTime());
                        const uint64_t overlapEnd =
                            std::min(childEndTime, newNodes[index].endTime());
                        // Connect if there is overlap
                        if (overlapStart < overlapEnd) {
                            addHierarchyEdge(newIndices[index], child);
                        }
                        // For single time value Children, there will only be a single edge
...
void TemporalTree::getLeaves(const size_t nodeIndex, const uint64_t initialStarttTime,
                             const uint64_t initialEndTime, uint64_t startTime, uint64_t endTime,
                             std::set<size_t>& Leaves) const {
    ...
    // Called on a leaf
    if (it == edgesHierarchy.end()) {
        // Get only leaves that are overlapping with start,end excluding only overlap on the
        // endpoints
    ...
    // Update the startime for this node (if for some reason function was called with times beyond
    // the nodes lifetime)
    ...
    for (const auto& childIndex : it->second) {
        ...
        if (TNode::isOverlappingTemporally(startTime, endTime, nodes[childIndex].startTime(),
                                           nodes[childIndex].endTime())) {
            if (edgesHierarchy.find(childIndex) == edgesHierarchy.end()) {
                // It overlaps with initial time, so we insert it

                if (initialStarttTime == initialEndTime ||
                    (initialStarttTime != nodes[childIndex].endTime() &&
                     initialEndTime != nodes[childIndex].startTime())) {
                ...
                getLeaves(childIndex, startTimeChild, endTimeChild, startTimeChild, endTimeChild,
                          Leaves);
            }
        }
    }
...
std::vector<size_t> TemporalTree::getEdgesFrom(const size_t nodeIndex,
                                               const TAdjacency& Edges) const {
...
std::vector<size_t> TemporalTree::getEdgesTo(const size_t nodeIndex,
                                             const TAdjacency& Edges) const {
...
void TemporalTree::deaggregate(TemporalTree& tree) const {
    ...
    for (auto node : nodes) {
        ...
        if (node.endTime() == *times.rbegin()) {
            ...
            TNode newNode(node.name + "_" + std::to_string(itValue->first),
        ...
        nodesOrgingalToDeaggregated.emplace_hint(nodesOrgingalToDeaggregated.end(), nodeIndex,
                                                 correspondingNodes);
    ...
    for (auto outgoingTemporalEdges : edgesTime) {
        ...
        if (it != nodesOrgingalToDeaggregated.end() && !it->second.empty()) {
            ...
            LogWarn("Temporal edge group not in deaggregated") continue;
        }
    ...
    for (auto outgoingHierarchyEdges : edgesHierarchy) {
        ...
        for (auto edgeTo : outgoingHierarchyEdges.second) {
            ...
            if (it != nodesOrgingalToDeaggregated.end()) {
                ...
                if (edgeFrom == 0) {
                    for (auto firstLevelNode : nodesTo)
                        tree.addHierarchyEdge(nodesFrom[0], firstLevelNode);
                    continue;
                }
            ...
            if (it != nodesOrgingalToDeaggregated.end()) {
                ...
                for (auto newLeaf : leafNodes) {
                    if (tree.edgesHierarchy.find(newLeaf) != tree.edgesHierarchy.end()) {
                        LogError(
                            "Deaggregation: Node corresponding to leaf in old tree is not leaf "
                            "anymore in new tree");
                    }
...
TemporalTree TemporalTree::aggregate() const {
    ...
    for (size_t nodeIndex = 0; nodeIndex < this->nodes.size(); nodeIndex++) {
        ...
        if (it == indexMap.end()) {
            // Copy the representative node (including all its values) from this tree to the
            // aggregated one
    ...
    for (auto it = this->edgesTime.begin(); it != this->edgesTime.end(); it++) {
        for (auto edgeTo : it->second) {
            ...
            if (correspondenceFrom != correspondenceTo) {
                // Check if there are already outgoing edges for the from node and either add an
                // entry or modfiy the entry
                ...
                if (itAggregated == aggregatedEdgesTime.end()) {
                    ...
                    aggregatedEdgesTime.insert(
                        std::make_pair(indexMap[correspondenceFrom], edgesTo));
                } else {
...
void TemporalTree::TNode::fillWithLeftNeighborInterpolation(const std::set<uint64_t>& times,
...
bool TemporalTree::TNode::isOverlappingTemporally(uint64_t startTimeA, uint64_t endTimeA,
                                                  uint64_t startTimeB, uint64_t endTimeB) {
    return
        //   |---|      (A)
        //|-|           (B) starts before A, ends before start of A: startTimeA > endTimeB (<=
        //yields false)
...
void TemporalTree::computeAccumulated(const size_t subtreeIndex, const std::set<uint64_t>& times,
    ...
    for (auto& child : this->getHierarchicalChildren(subtreeIndex)) {
        ...
        // but only where the time overlaps (maximum of the start times and minimum of the end
        // times)
        TemporalTree::TNode& childNode = nodes[child];

        auto childIt =
            childNode.values.find(std::max(subtreeNode.startTime(), childNode.startTime()));
        // Include the last value if this node does not split afterwards
        auto childItLast =
            childNode.values.find(std::min(subtreeNode.endTime(), childNode.endTime()));
...
std::vector<unsigned int> TemporalTree::computeComponents(const TAdjacency& Edges) const {
    ...
    std::function<void(size_t, bool, unsigned int)> processNode =
        [&](const size_t node, const bool forward, const unsigned int componentId) {
...
std::map<uint64_t, float> TemporalTree::computeAccumulatedRootOnly(
    const std::set<uint64_t>& times) const {
    ...
    for (auto leaf : leaves) {
        ...
        for (auto timeValuePair : values) {
            // If there are temporal sucessors, skip the value, the sucessors first value will
            // contribute to the sum
            if (timeValuePair.first == values.rbegin()->first &&
                !getTemporalSuccessors(leaf).empty()) {
                continue;
            }
...
size_t TemporalTree::getConstraintClusters(std::vector<std::pair<int, int>>& Clusters) const {
    ...
    for(const auto& edge : edgesTime)
    {
        ...
        ivwAssert(!idsToRight.empty(), "Time map is empty.");
        if (idsToRight.empty()) continue;
        ivwAssert(bVisitedLeft[idFromLeft] == bVisitedRight[idsToRight.front()], "Visitation maps
    should be consistent.");
        ...
        while (!Q.empty())
        {
            ...
            const std::vector<size_t>& ToTheGreenerSide = Front.first ? edgesTime.at(Front.second) :
    reversedEdgesTime.at(Front.second); std::vector<bool>& bVisitedOnTheGreenerSide = Front.first ?
    bVisitedRight : bVisitedLeft;
...
void TemporalTree::getMinMaxTimeShallow(const size_t nodeIndex, uint64_t& tMin,
                                        uint64_t& tMax) const {
    ...
    const std::vector<size_t>& Children =
        (itHierarchyEdges == edgesHierarchy.end()) ? JustMe : itHierarchyEdges->second;
...
void TemporalTree::addDefaultTimesForParents(const size_t nodeIndex, uint64_t& tMin,
                                             uint64_t& tMax) {
    // Get the children of this node
    const auto itHierarchyEdges = edgesHierarchy.find(nodeIndex);
...
bool TemporalTree::checkConsistency() const {
    ...
    if (nodesLessThanTwoValues)
        LogInfo("There are " << nodesLessThanTwoValues << " nodes with less than two values.");
    ...
    for (const auto& adjacencyNode : this->edgesTime) {
        ...
        for (auto edgeTo : adjacencyNode.second) {
            ...
            if (nodes[edgeFrom].endTime() != nodes[edgeTo].startTime()) {
                LogError("The temporal edge ["
                         << edgeFrom << "," << edgeTo << "] connects nodes with ending time"
                         << nodes[edgeFrom].endTime() << " and "
                         << " begin time " << nodes[edgeTo].startTime() << ".") return false;
            }
        }
    }
    ...
    std::vector<std::pair<uint64_t, uint64_t>> componentLifetimes(
        numComponents,
        std::make_pair(std::numeric_limits<uint64_t>::max(), std::numeric_limits<uint64_t>::min()));
    ...
    for (size_t component = 1; component < numComponents; component++) {
        ...
        if (componentLifetimes[parentComponent].first > componentLifetimes[component].first &&
            ...
            LogError(
                "The child components lifeTime does not overlap with the lifetime of the parent.");
            return false;
        }
    }
    ...
    for (const auto& adjacencyNode : this->edgesHierarchy) {
        ...
        for (auto edgeTo : adjacencyNode.second) {
            ...
            // If both parent and children have at least two values (i.e. start and end time are
            // different), then the maximum of the start times should be strictly smaller than the
            // minimum of the end times Special cases occur if parent of child lives only for a
            // single time step
    ...
    // Alternative: Look at aggregated parents of aggregated children and make sure that they are
    // connected by edges (Ensures both that the node always has a parent and we can check that the
    // lifetime of that parent is ok) Also we do not ensure that merged / (split can be treated the
    // same way) nodes have the same parent like this e.g. two siblings have children that merge but
    // they are in the same component because they also merge later We need to make sure that all
    // merged nodes and the merge result have the same parent and the time point of the merge
