...
void GetLayerOrder(const TemporalTree& Tree, const std::vector<size_t>& LeafOrder,
                   const std::map<size_t, size_t>& LeafOrderMap,
                   const std::vector<size_t>& LevelIndices,
                   TemporalTreeMeshGeneratorTopo::TLayerOrder& LayerOrder) {
    // Prepare memory
    ...
    for (const size_t idxParent : LevelIndices) {
        ...
        if (int(MaxOrderRow) - int(MinOrderRow) + 1 == NumOverlap) {
            for (auto it = LayerOrder.rbegin();
                 it != LayerOrder.rend() && it->idxNodeToBeDrawn == idxParent; it++) {
            ...
            for (size_t r(MinOrderRow); r <= MaxOrderRow && r < NumLeaves && NumOverlap >= 0; r++) {
                ...
                // then we do not have full coverage.
...
void TemporalTreeMeshGeneratorTopo::CreateMesh(const TemporalTree& Tree, const size_t Level,
                                               const size_t MaxLevel, const size_t NumRows,
                                               const TLayerOrder& Order,
    ...
    const float yLevelOffset =
        yActualBandWidth * (float(Level) / float(MaxLevel)) * (propLevelPortion / (2.0f * 100.0f));
    ...
    for (size_t i(0); i < NumItems; i++) {
        ...
        if (Constraints[idxThisNode].first >= 0) {
            vec3 LeftColor3 = color::hsv2rgb(
                vec3(float(Constraints[idxThisNode].first) / float(NumConstraints), 1.0f, 1.0f));
        ...
        if (Constraints[idxThisNode].second >= 0) {
            vec3 RightColor3 = color::hsv2rgb(
                vec3(float(Constraints[idxThisNode].second) / float(NumConstraints), 1.0f, 1.0f));
        ...
        Vertices[StartVertex + 2] = {pos, vec3(0, 0, 0), pos,
                                     bIndicateRupture ? RuptureColor : DefaultColor};
        pos = {xLeftIndication, yTop, float(Level)};
        Vertices[StartVertex + 3] = {pos, vec3(0, 0, 0), pos,
                                     bIndicateRupture ? RuptureColor : DefaultColor};
        pos = {xRightIndication, yBottom, float(Level)};
        Vertices[StartVertex + 4] = {pos, vec3(0, 0, 0), pos,
                                     bIndicateRupture ? RuptureColor : DefaultColor};
        pos = {xRightIndication, yTop, float(Level)};
        Vertices[StartVertex + 5] = {pos, vec3(0, 0, 0), pos,
                                     bIndicateRupture ? RuptureColor : DefaultColor};
