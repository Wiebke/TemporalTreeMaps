...
TemporalTreeGenerateFromFileSystem::TemporalTreeGenerateFromFileSystem()
    ...
    propAction.onChange([&]() { ScanFileSystem(); });
...
void RecordHistoricalSizes(const fs::path& ScanDir, std::map<uint64_t, float>& HistoricalSizes) {
    ...
    for (const fs::directory_entry& entry : fs::recursive_directory_iterator(ScanDir)) {
        ...
        if (fs::is_regular_file(entry.status())) {
            ...
            fs::file_time_type file_time = fs::last_write_time(entry);
            uint64_t FileModTime = static_cast<uint64_t>(to_time_t(file_time));
            // - add them
...
void ScanRecursively(const fs::path& ScanDir, const size_t idParent, const int Level,
                     const int MaxLevel, TemporalTree& OutTree) {
...
void GenerateTimeSpans(const size_t idNode, const uint64_t tMax, uint64_t& tMin,
                       TemporalTree& OutTree) {
...
void TemporalTreeGenerateFromFileSystem::ScanFileSystem() {
    ...
    //// - what is now?
    // uint64_t Now =
    // (uint64_t)std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()); tMax = Now;
    ...
    stMin << std::put_time(std::localtime(&ttMin),
                           "%Y-%m-%d %X");  // If you call them in the same stream, they will print
                                            // the first time only. STL bug?
    stMax << std::put_time(std::localtime(&ttMax), "%Y-%m-%d %X");
    LogInfo("File System Tree with " << OutTree.nodes.size() << " nodes in range: [" << tMin << ", "
                                     << tMax << "], which is " << stMin.str() << " through "
                                     << stMax.str() << ". Needed " << Timer.ElapsedTime()
                                     << " seconds.");
