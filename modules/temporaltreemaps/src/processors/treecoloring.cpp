...
const ProcessorInfo TemporalTreeColoring::getProcessorInfo() const { return processorInfo_; }
...
TemporalTreeColoring::TemporalTreeColoring()
    ...
    , propColorUniform("colorUniform", "Uniform Color", vec4(0.5f, 0.5f, 0.5f, 1.0f), vec4(0.0f),
    ...
    propColorSpace.onChange([&]() {
        if (propColorSpace.get() == 0) {
            ...
            util::hide(propHSVOffset, propColorBrewerScheme, propColorSeed, propSaturationMin,
                       propSaturationMax);
            util::show(propColorSpaceMap, propAlternate);
        } else {
            util::hide(propColorSpaceMap, propAlternate);
            util::show(propHSVOffset, propColorBrewerScheme, propColorSeed, propSaturationMin,
                       propSaturationMax);
        }
    });
    ...
    util::hide(propColorSeed, propValueTranserFunc, propColorBrewerScheme, propAlternate,
               propColorTo, propColorFrom, propColorSpace, propColorSpaceMap, propHSVOffset,
               propRangeDecay, propSaturationMin, propSaturationMax);

    propColorScheme.onChange([&]() {
        switch (propColorScheme.get()) {
            case 0:
                util::hide(propColorUniform, propColorSeed, propValueTranserFunc,
                           propColorBrewerScheme, propAlternate, propColorTo, propColorFrom,
                           propColorSpace, propColorSpaceMap, propHSVOffset, propRangeDecay);
                break;
            case 1:
                util::hide(propColorSeed, propValueTranserFunc, propColorBrewerScheme,
                           propAlternate, propColorTo, propColorFrom, propColorSpace,
                           propColorSpaceMap, propHSVOffset, propRangeDecay);
                util::show(propColorUniform);
                break;
            case 2:
                util::hide(propColorUniform, propColorSeed, propColorBrewerScheme, propAlternate,
                           propColorTo, propColorFrom, propColorSpace, propColorSpaceMap,
                           propHSVOffset, propRangeDecay);
            ...
            case 3:
                ...
                break;
            case 4:
                util::hide(propColorUniform, propValueTranserFunc, propAlternate, propColorTo,
                           propColorFrom, propColorSpace, propColorSpaceMap, propHSVOffset,
                           propRangeDecay);
                util::show(propColorBrewerScheme);
                break;
            case 5:
                util::hide(propColorUniform, propColorSeed, propValueTranserFunc,
                           propColorBrewerScheme);
                util::show(propAlternate, propColorTo, propColorFrom, propColorSpace,
                           propRangeDecay);
                if (propColorSpace.get() == 0) {
                    util::hide(propColorSpaceMap, propColorBrewerScheme, propColorSeed,
                               propSaturationMin, propSaturationMax);
                    util::show(propHSVOffset);
                }
                if (propColorSpace.get() == 1) {
                    util::hide(propHSVOffset, propColorBrewerScheme, propColorSeed,
                               propSaturationMin, propSaturationMax);
...
void TemporalTreeColoring::process() {
    ...
    if (propColorScheme.get() == 5) {
        ...
        if (propColorSpace == 0) {
            ...
            auto colorBrewerMap =
                colorbrewer::getColormap(propColorBrewerScheme, (glm::uint8)numFirstLevel);
            ...
            for (size_t inFirstLevelIndex = 0; inFirstLevelIndex < firstLevel.size();
                 inFirstLevelIndex++) {
            ...
            sampleColor = [this, &numFirstLevel, &colorMap](float x) -> vec3 {
                ...
                float s = propSaturationMin * (float(colorIndex + 1) / numFirstLevel - x) /
                              (1.0f / numFirstLevel) +
                          propSaturationMax * (x - float(colorIndex) / numFirstLevel) /
                              (1.0f / numFirstLevel);
        ...
        treecolor::traverseToLeavesForColor(
            *pOutTree,
            0,     // nodeIndex
            0, 1,  // range
            propColorSpace != 2 ? propAlternate : false, pOutTree->nodes[0].startTime(),
            pOutTree->nodes[0].endTime(),
            0,  // depth
            propRangeDecay, propColorFrom, propColorTo, sampleColor);
    ...
    for (auto leaf : leaves) {
        ...
        for (auto itLowerLimit = lowerLimitLeaf.begin(), itUpperLimit = upperLimitLeaf.begin();
            ...
            switch (propColorScheme.get()) {
                ...
                case 2:
                    colors.emplace_hint(
                        colors.end(), itLowerLimit->first,
                        propValueTranserFunc.get().sample(
                            itLowerLimit == lowerLimitLeaf.begin()
                ...
                case 3:
                    ...
                    // by setting the seed based on the index (which does not change regardless of
                    // order)
                    randomGen.seed(
                        static_cast<std::mt19937::result_type>(leaf + propColorSeed.get()));
                    colors.emplace_hint(
                        colors.end(), itLowerLimit->first,
                        vec4(rand(0.0f, 1.0f), rand(0.0f, 1.0f), rand(0.0f, 1.0f), 1.0f));
                    break;
                case 4:
                    colors.emplace_hint(colors.end(), itLowerLimit->first, colorMap[leafCounter]);
                default:
                    colors.emplace_hint(colors.end(), itLowerLimit->first,
