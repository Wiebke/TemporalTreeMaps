...
const ProcessorInfo TemporalTreeFilter::getProcessorInfo() const { return processorInfo_; }
...
TemporalTreeFilter::TemporalTreeFilter()
    ...
    , propEndLifespan("endLifeSpan", "End Lifespan", 100)
...

{
    ...
    propDoFilterFirstLevel.onChange([this]() {
        ...
        if (propDoFilterFirstLevel.get()) {
            ...
            for (auto& nodeIndex : firstLevelNodes) {
                ...
                auto bp = new BoolProperty(
                    "prop" + std::to_string(nodeIndex),
                    pInTree->nodes[nodeIndex].name + " - " + std::to_string(int(maxValue)),
                    maxValue < 0.015 * maxValueTree);
    ...
    propDoFilter.onChange([this]() {
        if (propDoFilter.get() == 0) {
            util::hide(propMinLifespan, propStartLifespan, propEndLifespan, propFilterDepth,
                       propFilterFirstLevelNodes);
            // Remove any old properties
            auto properties = propFilterFirstLevelNodes.getProperties();
            for (auto& prop : properties) {
                propFilterFirstLevelNodes.removeProperty(prop);
            ...
            util::show(propMinLifespan, propStartLifespan, propEndLifespan, propFilterDepth,
                       propFilterFirstLevelNodes);
        }
    });
...
void TemporalTreeFilter::fadeLeaves(std::shared_ptr<TemporalTree>& tree) {
    ...
    for (auto leaf : leaves) {
        ...
        // If this node truly appears, its beginning value is not 0 and the time is not the global
        // beginning
        if (tree->getTemporalPredecessorsWithReverse(leaf).size() == 0 &&
            leafNode.startTime() != *times.begin()) {
            // Minimum out of global end, leaf end and leaf befin + delta fade
            uint64_t fadeInEnd =
                std::min(std::min(leafNode.startTime() + propDeltaTFade.get(), leafNode.endTime()),
                         *times.rbegin());
        ...
        if (tree->getTemporalSuccessors(leaf).size() == 0 &&
            leafNode.endTime() != *times.rbegin()) {
            // Maximum out of global begin, leaf begin and leaf end - delta fade (We need to treat
            // potential unsigned int underflow here)
            uint64_t fadeOutBegin =
                std::max(std::max((leafNode.endTime() - propDeltaTFade.get()) *
                                      (leafNode.endTime() >= propDeltaTFade.get()),
                                  leafNode.startTime()),
                         *times.begin());
    ...
    for (auto leaf : fadeIn) {
        ...
        if (tree->getTemporalPredecessorsWithReverse(leaf).size() == 0 &&
            ...
            uint64_t fadeInEnd =
                std::min(leafNode.startTime() + propDeltaTFade.get(), *times.rbegin());
            ...
            // (but not longer then the lifespan, so that we do not add additional values, parents
            // might not be active either )
            auto itEnd = std::next(times.find(std::min(fadeInEnd, leafNode.endTime())));
            for (auto itTimes = times.find(leafNode.startTime()); itTimes != itEnd; itTimes++) {
    ...
    for (auto leaf : fadeOut) {
        ...
        if (tree->getTemporalSuccessors(leaf).size() == 0 &&
            ...
            uint64_t fadeOutBegin = std::max((leafNode.endTime() - propDeltaTFade.get()) *
                                                 (leafNode.endTime() >= propDeltaTFade.get()),
                                             *times.begin());
            // If the lifespan of the node is shorter than the fading out timespan, it will just be
            // 0
...
void TemporalTreeFilter::filterLeaves(std::shared_ptr<TemporalTree>& tree) {
    ...
    for (auto& node : tree->nodes) {
        ...
        if (propDoFilterFirstLevel && depth == 1 &&
            !propFilterFirstLevelNodes.getProperties().empty()) {
            auto pp = dynamic_cast<BoolProperty*>(propFilterFirstLevelNodes.getPropertyByIdentifier(
                "prop" + std::to_string(nodeIndex)));
            if (pp && pp->get()) {
                removeNodes.insert(nodeIndex);
        ...
        for (auto it = times.find(propStartLifespan); it != times.upper_bound(propEndLifespan);
             it++) {
            uint64_t time = *it;
...
float TemporalTreeFilter::linearInterpolation(const std::pair<uint64_t, float>& left,
                                              const std::pair<uint64_t, float>& right,
                                              const uint64_t x) {
    ...
    return left.second +
           float(x - left.first) * (right.second - left.second) / float(right.first - left.first);
}
