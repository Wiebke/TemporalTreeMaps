...
void TemporalTreeOrderComputationSAEdges::restart() {
    ...
    chooseNode = std::uniform_int_distribution<int>(0, static_cast<int>(activeNodes.size()) - 1);

    if (!activeNodes.empty()) {
        LogInfo("Active nodes: " << activeNodes.size()
                                 << " with average degree: " << averageDegree / activeNodes.size());
    }
...
void TemporalTreeOrderComputationSAEdges::dfsTime(std::shared_ptr<const TemporalTree> tree,
                                                  size_t nodeIndex, std::vector<bool>& visited) {
...
void TemporalTreeOrderComputationSAEdges::dfsHierarchy(std::shared_ptr<const TemporalTree> tree,
                                                       size_t nodeIndex,
                                                       std::vector<bool>& visitedHierarchy,
                                                       std::vector<bool>& visitedTime) {
            ...
            if (!visitedHierarchy[child]) {
                if (!visitedTime[child]) {
                    if (initCase != MergesAndSplits) {
                        ...
                        // This node has multiple parents, i.e. is child of something that splits or
                        // merges
                        if (tree->reverseEdgesHierachy.at(child).size() > 1) {
                            // We add it to the parent of its parent in the heuristic tree
                            // It does not matter for which of its parents we look this up, they all
                            // shoudl have the same parent
    ...
    for (size_t nodeIndex(0); nodeIndex < tree->nodes.size(); nodeIndex++) {
        ...
        if (itOptimization == currentEdges.end()) {
            ...
            if (itOriginal != tree->edgesHierarchy.end()) {
                ...
                // Delete the edge
                auto itToDelete =
                    std::find(outgoingFromParent.begin(), outgoingFromParent.end(), nodeIndex);
