...
const ProcessorInfo TemporalTreeStatistics::getProcessorInfo() const { return processorInfo_; }
...
namespace {
void GenerateStatisticsString(const size_t NumLevels, const size_t NumLeaves, const uint64_t tMin,
                              const uint64_t tMax, const size_t NumNodes,
                              const size_t NumHierarchyEdges, const size_t NumTimeEdges,
                              const size_t NumTimeSteps, StringProperty& StatString) {
    std::stringstream ss;

    ss << "Number of Nodes: " << NumNodes << std::endl
    ...
    const size_t TotalBytes =
        NumNodes * BytesPerNode + (NumHierarchyEdges + NumTimeEdges) * BytesPerEdge;
    ss << "Memory: " << TotalBytes << " Bytes (" << float(TotalBytes) / (1024. * 1024.) << " MB)"
       << std::endl;
...
void TemporalTreeStatistics::process() {
    ...
    GenerateStatisticsString(MaxLevel, NumLeaves, tMin, tMax, NumNodes, NumHierarchyEdges,
                             NumTimeEdges, AllTimeSteps.size(), propStatGivenTree);
...
#endif
    //#pragma omp parallel for reduction(+: NANumNodes, NANumHierarchyEdges, NANumTimeEdges,
    //NANumLeaves)
    for (signed long long i(0); i < iNumNodes; i++) {
        ...
        for (const size_t& idxChild : Children) {
            ...
            const size_t NumAggregatedChildTimeStepHops =
                std::distance(itChildNodeStart, itChildNodeEnd);

            NANumHierarchyEdges += NumAggregatedChildTimeStepHops + 1;
        }
    }

    GenerateStatisticsString(MaxLevel, NANumLeaves, tMin, tMax, NANumNodes, NANumHierarchyEdges,
                             NANumTimeEdges, AllTimeSteps.size(), propStatNonAggregatedTree);
}
