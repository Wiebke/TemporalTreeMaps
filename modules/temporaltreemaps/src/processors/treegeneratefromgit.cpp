...
const ProcessorInfo TemporalTreeGenerateFromGit::getProcessorInfo() const { return processorInfo_; }
...
TemporalTreeGenerateFromGit::TemporalTreeGenerateFromGit()
    ...
    propAction.onChange([&]() { ScanGitRepository(); });
...
void TemporalTreeGenerateFromGit::RunGitLog(const fs::path& GitRepo, TGitCommitLog& Log) {
    ...
    // std::string GitCommand = "cd " + propGitRepoDir.get() + "; git log
    // --pretty=format:commit:%H%ntimestamp:%ct --reverse --no-renames --raw";
    std::string GitCommand =
        "git log --pretty=format:commit:%H%ntimestamp:%ct --reverse --no-renames --raw";
    if (!propCommitsSince.get().empty())
        GitCommand += " --since \"" + propCommitsSince.get() + "\"";
    if (!propCommitsUntil.get().empty())
        GitCommand += " --until \"" + propCommitsUntil.get() + "\"";
    ...
    while (std::getline(TextLog, CurrentLine)) {
        ...
        if (idxCommit != std::string::npos) {
            ...
            CurrentCommit.Sha1 =
                CurrentLine.substr(idxCommit + 7 /*std::string("commit:").length()*/);
        ...
        if (idxLastTab != std::string::npos) {
            ...
            if (idxStartChange != std::string::npos) {
                ...
                switch (CurrentLine[idxStartChange + 1]) {
                    ...
                    case 'C':
                    case 'M':
                        WhatHappened = EGitFileChange::Modified;
                        break;

                    case 'A':
                        WhatHappened = EGitFileChange::Created;
                        break;

                    case 'D':
                        WhatHappened = EGitFileChange::Deleted;
                        break;

                    default:
                        WhatHappened = EGitFileChange::Undefined;
                        break;
                }
                ...
                strFileName.erase(strFileName.begin(),
                                  std::find_if(strFileName.begin(), strFileName.end(),
...
void ScanRecursively(const fs::path& ScanDir, const size_t idParent, const uint64_t Timestamp,
    ...
    for (const fs::directory_entry& entry : fs::directory_iterator(ScanDir)) {
        // Empty file or directory? We do not record them. Empty directories make trouble in
        // particular.
...
bool RunSystemCmd(const std::string& Cmd) {
    ...
    if (sysret != 0) {
        LogErrorCustom("TemporalTreeGenerateFromGit", "Error when running \'"
                                                          << Cmd << "\'. Process exited with "
                                                          << sysret << ".");
        return false;
    }
...
void TemporalTreeGenerateFromGit::GitLogToTree(const TGitCommitLog& Log,
                                               const fs::path& GitRepoBase, TemporalTree& Tree) {
    ...
    for (size_t i(1); i < Log.size(); i++) {
        ...
        for (const auto& FileAndWhat : ThisCommit.Files) {
            ...
            // - size
            const float FileSizeKB(bFoundOnDisk && bIsFile ? GetFileSizeKBAsFloat(FileAndWhat.first)
                                                           : 0.f);
            ...
            // Compare to commit status - this is only informative. We ultimately record the disk
            // status below.
            if (FileAndWhat.second == EGitFileChange::Created && (!bFoundOnDisk || bFoundInTree)) {
                LogWarnCustom("TemporalTreeGenerateFromGit",
                              "File should have been newly created, but OnDisk = "
                                  << bFoundOnDisk << " and InTree = " << bFoundInTree
                                  << " for file " << FileAndWhat.first << " at commit "
                                  << ThisCommit.Sha1);
            }
            if (FileAndWhat.second == EGitFileChange::Modified &&
                (!bFoundOnDisk || !bFoundInTree)) {
                LogWarnCustom("TemporalTreeGenerateFromGit",
                              "File should have been modified, but OnDisk = "
                                  << bFoundOnDisk << " and InTree = " << bFoundInTree
                                  << " for file " << FileAndWhat.first << " at commit "
                                  << ThisCommit.Sha1);
            }
            if (FileAndWhat.second == EGitFileChange::Deleted && (bFoundOnDisk || !bFoundInTree)) {
                LogWarnCustom("TemporalTreeGenerateFromGit",
                              "File should have been deleted, but OnDisk = "
                                  << bFoundOnDisk << " and InTree = " << bFoundInTree
                                  << " for file " << FileAndWhat.first << " at commit "
                                  << ThisCommit.Sha1);
            }
            ...
            if (bFoundOnDisk && bIsFile) {
                if (bFoundInTree) {
                    ...
                    // - remove the common base path. Could be done with relative paths, but I want
                    // to avoid ".." and rather run into an error.
                    ...
                    for (; itBase != GitRepoBase.end(); itBase++, itCurrent++) {
                        ivwAssert(itCurrent != FileAndWhat.first.end(),
                                  "File path outside of repository base path.");
                        ivwAssert(*itBase == *itCurrent,
                                  "File path outside of repository base path.");
                    }
                    ...
                    do {
                        const size_t idCurrent =
                            Tree.addChild(idParent, Current.filename().string(), {});
...
void TemporalTreeGenerateFromGit::AggregateGitLog(const TGitCommitLog& Log,
    ...
    for (size_t i(0); i < numCommits; i++) {
        ...
        for (const auto& FileAndWhat : Log[i].Files) {
            ...
            auto itFile =
                std::find_if(AggregatedLog.back().Files.begin(), AggregatedLog.back().Files.end(),
                             [&](const auto& s) { return s.first == FileAndWhat.first; });
...
void TemporalTreeGenerateFromGit::AggregateGitLogTags(const TGitCommitLog& Log,
                                                      TGitCommitLog& AggregatedLog,
    ...
    for (size_t i(0); i < Log.size(); i++) {
        ...
        // Start a new log entry if: the commit is a tag, the commit is first (but not if it is the
        // last one)
...
void TemporalTreeGenerateFromGit::ScanGitRepository() {
    ...
    if (GitRepoString.empty() || !fs::is_directory(GitRepo) ||
        !fs::is_directory(GitRepo / ".git")) {
        LogError(
            "Please provide the path to a proper git repository. The \'.git\' folder needs to be a "
            "subdirectory directly under the given one.");
        return;
    }
    ...
    LogInfo("Read " << Log.size() << " commits from the repository. Needed " << Timer.ElapsedTime()
                    << " seconds.");
    ...
    if (propCommitsChoice.get() == 1) {
        ...
        if (sysret != 0) {
            ...
            while (std::getline(TextLog, CurrentLine)) {
                // Take all tags, dereferences and not (annotated tags will occur with both commit
                // and tag)
    ...
    if (!AggregatedLog.empty()) {
        ...
        time_t ttMin = tMin;
        time_t ttMax = tMax;
        stMin << std::put_time(std::localtime(&ttMin),
                               "%Y-%m-%d %X");  // If you call them in the same stream, they will
                                                // print the first time only. STL bug?
    ...
    LogInfo("Git temporal tree with " << OutTree.nodes.size() << " nodes in range: [" << tMin
                                      << ", " << tMax << "], which is " << stMin.str()
                                      << " through " << stMax.str() << ". Needed "
