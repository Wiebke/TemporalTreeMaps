...
void TemporalTreeOrderComputationHeuristic::findConflictingLeaves(
    std::shared_ptr<const TemporalTree>& tree, const Constraint& constraint,
    const TemporalTree::TTreeOrder& order, size_t& minOrder, size_t& maxOrder,
...
void TemporalTreeOrderComputationHeuristic::buildNewOrder(
    TemporalTree::TTreeOrder& newOrder, const TemporalTree::TTreeOrder& order,
    const size_t numConflictBefore, const TemporalTree::TTreeOrder& conflictingLeaves,
    const TemporalTree::TTreeOrder& nonConflictingAndConstraintLeaves, const size_t minOrder,
    ...
    newOrder.insert(newOrder.end(), conflictingLeaves.begin(),
                    conflictingLeaves.begin() + numConflictBefore);

    // Insert all nonconflicting and constraint nodes
    newOrder.insert(newOrder.end(), nonConflictingAndConstraintLeaves.begin(),
                    nonConflictingAndConstraintLeaves.end());

    // Insert all conflighting leaves that are left now
    newOrder.insert(newOrder.end(), conflictingLeaves.begin() + numConflictBefore,
                    conflictingLeaves.end());
...
bool TemporalTreeOrderComputationHeuristic::resolveConstraint(
    ...
    findConflictingLeaves(pInputTree, constraint, currentState.order, minOrder, maxOrder,
    ...
    for (int numConflictBefore(int(conflictingLeaves.size())); numConflictBefore >= 0;
         numConflictBefore--) {
        buildNewOrder(temporaryOrder, currentState.order, numConflictBefore, conflictingLeaves,
    ...
    buildNewOrder(temporaryOrder, currentState.order, bestIds[solutionId], conflictingLeaves,
                  nonConflictingAndConstraintLeaves, minOrder, maxOrder);
        ...
        case 2:
            ...
            std::sort(constraintOrder.begin(), constraintOrder.end(),
                      [&](const size_t a, const size_t b) -> bool {
                          ...
                          if (levelA == levelB) {
                              if (constraints[a].type == ConstraintType::Hierarchy &&
                                  constraints[b].type != ConstraintType::Hierarchy)
                                  return true;
                              else if (constraints[a].type != ConstraintType::Hierarchy &&
                                       constraints[b].type == ConstraintType::Hierarchy)
                                  return false;
                              else
                                  return sizeA > sizeB;
...
void TemporalTreeOrderComputationHeuristic::singleStep() {
    ...
    // resolve There might be constraint in the queue that are already fulfilled, an interation
    // fulfills another constraint
    while (!isConverged() &&
           !resolveConstraint(portInTree.getData(), constraints[constraintsQueue.front()])) {
        // If resolve Constraint was true we have to pop the fulfilled constraint
    ...
    // Pop constraint we just resolved
    if (!constraintsQueue.empty()) constraintsQueue.pop();
}
...
void TemporalTreeOrderComputationHeuristic::prepareNextStep() {
    // If the queue is not empty, we have just processed a step
    if (!constraintsQueue.empty()) constraintsQueue.pop();
...
void TemporalTreeOrderComputationHeuristic::logStep() { TemporalTreeOrderOptimization::logStep(); }
...
void TemporalTreeOrderComputationHeuristic::logProperties() {
    const std::vector<std::string> colHeaders{
        ...
        propIterationsMax.getDisplayName(),    propInitialConstraintOrder.getDisplayName(),
        propWeightByTypeOnly.getDisplayName(), propWeightTypeOnly.getDisplayName(),
        propBestIteration.getDisplayName(),    propObjectiveValue.getDisplayName(),
        propTimeUntilBest.getDisplayName(),    propTimeForLastAction.getDisplayName()};

    const std::vector<std::string> exampleRow{
        std::to_string(propSeedOrder),        std::to_string(propSeedOptimization),
        std::to_string(propIterationsMax),    std::to_string(propInitialConstraintOrder.get()),
        std::to_string(propWeightByTypeOnly), std::to_string(propWeightTypeOnly),
        std::to_string(bestState.iteration),  std::to_string(bestState.value),
        std::to_string(propTimeUntilBest),    std::to_string(propTimeForLastAction)};
