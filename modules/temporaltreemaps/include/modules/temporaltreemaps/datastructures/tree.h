...
public:
    ...
    struct TNode {
        ...
        /// Simple constructor
        TNode() : name(""), values(), lowerLimit(), upperLimit(), cushion(), colors() {}

        /// Element constructor
        TNode(const std::string& argname, const std::map<uint64_t, float>& argvalues = {})
            : name(argname), values(argvalues), lowerLimit(), upperLimit(), cushion(), colors() {}
        ...
        static void fillWithLeftNeighborInterpolation(const std::set<uint64_t>& times,
                                                      std::map<uint64_t, float>& values);

        static bool isOverlappingTemporally(uint64_t startTimeA, uint64_t endTimeA,
...
public:
    ...
    */
    ...
    /// Optionally give nodes of a previous level to avoid recomputation when iterating through
    /// levels
    std::vector<size_t> getLevel(const size_t level, const std::vector<size_t>& prevLevelIndices,
                                 const size_t lastLevel) const;

    /// Returns the number of levels from the given node down to the lowest leaf. If this node is a
    /// leaf, the result is 0.
    size_t getNumLevels(const size_t nodeIndex) const;

    void splitTemporaryLeaves();

    void leafDepthStatistic(const size_t nodeIndex, std::vector<bool>& visited,
                            const TAdjacency& edges, std::vector<size_t>& leafDepthCounters,
                            size_t depth) const;
    ...
    void getLeaves(const size_t nodeIndex, const uint64_t initialStartTime,
                   const uint64_t initalEndTime, uint64_t startTime, uint64_t endTime,
    ...
    /// Get the hierarchical parents of a node given by its index. Parentship might change over
    /// time.
    ...
    size_t getNumHierarchyEdges() const { return getNumEdges(edgesHierarchy); }

    /// Returns number of time edges
    size_t getNumTimeEdges() const { return getNumEdges(edgesTime); }
...
public:
    ...
    */
    ...
    size_t addChild(const size_t parent, const std::string& name,
                    const std::map<uint64_t, float>& values) {
    ...
    /// Adds default parent timings with zero as data value in the given subtree. @returns the time
    ...
    /// We can specify a set of nodes that are not updated (usually ones that are not active
    /// anymore)
    ...
    */
    ...
    /** Consistency check of the entire tree.
        ...
        - Make sure only siblings split and merge, i.e., have the same parent and start/end at the
       same time).
    ...
    */
    ...
    TemporalTree getHierarchyAt(const uint64_t time, const bool accumulate, const uint64_t
    deltaTime) const;
...
struct DataTraits<kth::TemporalTree> {
    ...
    static Document info(const kth::TemporalTree& data) {
        ...
        oss << "Tree with " << data.nodes.size() << " nodes, " << data.edgesHierarchy.size()
            << " hierarchical edge groups, " << data.edgesTime.size()
            << " temporal edge groups, and lifetime [" << data.nodes[0].startTime() << ", "
            << data.nodes[0].endTime() << "].";
        Document doc;
