...
protected:
    ...
    float linearInterpolation(const std::pair<uint64_t, float>& left,
                              const std::pair<uint64_t, float>& right, const uint64_t x);

    /// Filter out leaves
    void filterLeaves(std::shared_ptr<TemporalTree>& tree);

    /// Split up nodes that have temporally become leafes, so that in that time, they are actual
    /// leaves
